...
int dataSize = 0;
string[] dataInFile;
string[] salesDate = new string[physicalSize];
double[] salesAmnt = new double[physicalSize];
...
while(isNotValid) {
  try {
    ...
    userChoice = Prompt("\nEnte Menu Choice (Type 'M' to display menu): ").ToUpper();
...
void saveData() {
  ...
  for(int i = 1; i < dataSize; i++) {
    items[i] = $"{salesDate[itemIndex]}, {salesAmnt[itemIndex]}";
    itemIndex++;
  }
...
void displayData() {
  if(dataSize == 0) {
    ...
    for (int i = 0; i < dataSize; i++) {
      Console.WriteLine("{0,4} {1,-15} {2,10:f2}", "["+i+"]", salesDate[i], salesAmnt[i]);
    }
  }
}
    ...
    while(true) {
      try { 
        ...
        if(dataIndex >=0 && dataIndex < dataSize) {
          Console.WriteLine($"\nYou are editing this data: \n{salesDate[dataIndex],-15} {salesAmnt[dataIndex], 10:c2}");
...
void createGraph() {
  ...
  Array.Sort(salesDate, salesAmnt, 0, dataSize);
  ...
  while(dollars >= 0 ) {
    ...
    // for(int i = 0; i < dataSize; i++) {
    //   string[] salesDay = salesDate[i].Split('-');
    ...
    //   }
    // }
    string[] salesDay = salesDate[0].Split('-');

    for(int i = 1; i <= physicalSize; i++) {
      string formatDay = i.ToString("00");
      int dayIndex = Array.IndexOf(salesDate, $"{salesDay[0]}-{formatDay}-{salesDay[2]}"); 
